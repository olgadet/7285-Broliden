---
title: "Model: comparing BV groups, address no DEGs in V2"
format: html
...
# define script name
script_name <- "model-compare-BV-groups-address-no-DEGs-in-V2"
...
## Venn diagram
Checking overlap of DEGs found in V3 (adj.p.value < 0.05) and transcripts in V2 (p.value < 0.05)
...
```{r}
#| label: fig-venn
#| fig-cap: "Venn diagram of DEGs in V3 (adj. p.val < 0.05) and V2 (p.val < 0.05)"
...
#| tbl-cap: "Number of concordant and discordant pairs of DEGs in V3 and V2, adj.p.val < 0.05 and p.val < 0.05 respectively."

# calculate number of concordant pairs based on DEGs.B2_vs_B0 and DEGs.B2_vs_B0.unadj
...
# Fit V2 model based on V3 DEGs

```{r}
#| label: fig-V2-model-V3-DEGs
...
# keep only transcripts detected as DEGs in V3
idx <- match(degs_v3, rownames(data_cds_flr$counts))
idx <- na.omit(idx)
data_cds_flr <- data_cds_flr[idx,,keep.lib.sizes=FALSE]
print(length(idx))
...
## Number of DEGs
```{r}
#| label: tbl-degs-v2-degs-v3
#| tbl-cap: "Number of differentially down- and up-regulated transcripts, adjusted p-value < 0.05, no lfc threshold. Model fitted on transcripts found in V3."
...
```{r}
#| label: tbl-degs-unadjusted-v2-degs-v3
#| tbl-cap: "Number of differentially down- and up-regulated transcripts, unadjusted p-value < 0.05, no lfc threshold. model fitted on transcripts found in V3."
